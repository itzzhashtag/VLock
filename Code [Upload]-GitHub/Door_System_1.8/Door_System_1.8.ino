...
// --- Matrix Pins ---
//===============================================
#define KP_SDO     2             // SDO to interrupt pin 2 (INT0) or pin 3 (INT1)
#define KP_SCL     3             // SCL to any DIGITAL IO pin
...
const int DoorOpen = 180, DoorClose = 95;
...
void setup()
{
  ...
  // Init Keypad
  KP_16.begin(KP_SCL, KP_SDO);
  attachInterrupt(digitalPinToInterrupt(KP_SDO), keyChange, FALLING);
  Serial.println("✅ Initiated : KP_16 S-Mode 16-keys ");
  delay(150);
...
void keygen()
{
  ...
  if (KP_16.keyChange)
  {
    if (KP_16.GetKey16())
    {
      uint8_t keyNum = KP_16.GetKey16();
      const char keyMap[] = " 123A456B789C*0#D"; // simpler & clearer
      mappedKey = keyMap[keyNum];
      Serial.print("🔢 KEY TOUCH DETECTED ON PAD : ");
      Serial.print(keyNum);
      Serial.print("  --> Mapped key: ");
      Serial.println(mappedKey);
      delay(250);
    }
    KP_16.keyChange = false;
  }
}
//===============================================
